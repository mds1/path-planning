...
Real-time path planning in unknown terrain has not been studied as extensively and fewer solutions exist. In this paper a new path planning algorithm is presented, called Hierarchical D* Lite (HD*), that is capable of computing near-optimal paths fast enough to operate in real-time. HD* is based on D* Lite~\cite{dstar1} and Hierarchical Path-Finding A*~\cite{hpa}, but introduces numerous changes to improve performance and path quality. D* Lite was selected as the foundation of this algorithm for a few reasons:

\begin{enumerate}
  \item It guarantees an optimal path can be found with an appropriate heuristic
  \item The trade-off between optimality and computation time can be easily adjusted by modifying this heurisitic
  \item The algorithm is easy to understand, and thus easy to build upon
\end{enumerate}
