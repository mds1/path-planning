...
The result is a new heuristic-based planning algorithm capable of computing near-optimal paths fast enough to operate in real-time in unknown environments. HD* also provides flexibility on the trade-off between path optimality and computation time. Furthermore, because it is algorithmically simple, it is easy to extend or modify. This means additional features, such as accounting for vehicle dynamics can be accounted for by altering pre-existing functions.
